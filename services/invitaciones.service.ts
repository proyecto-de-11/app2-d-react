 ...
 */
export async function enviarSolicitudUnirse(
  invitacionData: CrearInvitacionDTO
): Promise<Invitacion> {
  const urlCompleta = `${INVITACIONES_BASE_URL}/api/invitaciones`;

  console.log(
    "📨 Enviando solicitud para unirse al equipo:",
    invitacionData.equipoId
  );
...
export async function obtenerInvitacionesEquipo(
  ...
  try {
    ...
    const response = await axios.get<
      import("@/types/invitacion-types").InvitacionesPaginadas
    >(urlCompleta, {
      headers: {
        Authorization: `Bearer ${token}`,
        accept: "*/*",
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 401) {
        ...
        throw new Error(
          "No tienes permisos para ver las invitaciones de este equipo."
        );
      }
    ...
    console.error("❌ Error inesperado:", error);
    throw new Error(
      "Ocurrió un error desconocido al obtener las invitaciones."
    );
  }
}
...
export async function responderInvitacion(
  respuestaData: ResponderInvitacionDTO
): Promise<Invitacion> {
  const urlCompleta = `${INVITACIONES_BASE_URL}/api/invitaciones/${respuestaData.id}/respuesta`;

  console.log(
    `📨 Respondiendo invitación ${respuestaData.id} con estado: ${respuestaData.nuevoEstado}`
  );
  ...
  try {
    ...
    if (respuestaData.nuevoEstado === "ACEPTADA") {
      console.log("agregando un nuevi miembro al chat");

      const resutl = await agregarMiembroAlChat(
        response.data.equipo.id,
        respuestaData.usario
      );
...
export async function agregarMiembroAlChat(
    equipoId: number,
    usuarioId: number
  ): Promise<RespuestaAgregarMiembro | undefined> {
    console.log("agregando miembro al equiposcon ide de equipo :", equipoId);

    try {
      const response = await axios.post<RespuestaAgregarMiembro>(
        `https://apimensajeria.onrender.com/api/chats/add/member`,
        {
          grupoId: equipoId,
          member: usuarioId,
        }
